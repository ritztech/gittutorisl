...
$result13 = mysql_query("SELECT `billno`, `itemid`, `qty`, `total`, `mrp`, `sprice`, `itemname`, `grpid`,weight FROM `sell_items` WHERE billno = $id");
...
<tr> <td align = "left"> Cash : MEMO  </td>   <td align = "right"> NAME:  <?php echo $row1['cust_name']?> </td>   </tr>
...
if($row1['dis_rs'] > 0){?> Dis Rs:       <?php  echo $row1['dis_rs']; }  ?>

<?php if($row1['dis_pr'] > 0){ ?> Dis %:  <?php  echo $row1['dis_pr']; }  ?>



</br> <u> <font size = "6">Net Amount  : <?php echo $row1['netpay']?>  </font></u>  </br>
...
UNDER THE SCHEME OF COMPOSITION
