...
$result1 = mysql_query("SELECT   DATE_FORMAT(sell_date,'%d/%m/%Y') as sell_date, `total`, `netpay`, `dis_rs`, `cust_id`, `cust_name`, `salesman_id`, `salesman_name`,
 `paid`, `balance`, `dis_pr`, `accid`, `billno` FROM `sell_invoice` where billno  = $id");
 
$row_pinv = mysql_fetch_array($result1);

$accc_id = $row_pinv['accid'];



$result_acc = mysql_query("SELECT `name`FROM `customers` WHERE cid = $accc_id");
$row_acc = mysql_fetch_array($result_acc);


$accc_naame = $row_acc['0'];
...
<script language="javascript" type="text/javascript" src="jscode/bill_items_e.js">  </script>
...
<form  id="form2" name = "form2" method="post"    action="bill_add_back_edit.php">

<br>

<div  align = "center">  <b> EDIT  SALE INVOICE : <?php echo $id  ?> </b>  </div>
...
<tr style="background-color:#000000;  color:#FFF;">  <td colspan = 2 align = "center">
 Total <input type = "text" name = "stotal" size="8" readonly style="background-color:#BB1013; color:#F1ECEC; font-size:13px; font-weight:bold;"   value = "<?php  $row_pinv['total']  ?>" id = "stotal"/>
 
 	%.Dis:<input type = "text" onKeyUp="shappycode1()" name = "sdis" size = "3" value = "<?php echo  $row_pinv['dis_pr']  ?>"  id = "sdis"/> 
	Rs .Dis:<input type = "text" onKeyUp="shappycode1()" name = "rsdis"  size = "3"  value = "<?php   echo $row_pinv['dis_rs']  ?>"  id = "rsdis"/> 
	
 
 
 NET PAY <input type = "text" name = "netpay"  value = "<?php echo  $row_pinv['netpay']  ?>"  size="8" readonly style="background-color:#BB1013; color:#F1ECEC; font-size:13px; font-weight:bold;"  id = "netpay"/>
 PAID<input type = "text" onKeyUp="balancecalc()"   name = "paid" size="5"   value = "<?php   echo  $row_pinv['paid']  ?>"   id = "paid"/>
 BALANCE <input type = "text" readonly name = "balance" size="5" value = "<?php echo  $row_pinv['balance']  ?>"  id = "balance"/>
 DATE : </b><input type = "text" size = "8" name = "sdate" value = "<?php   echo  $row_pinv['sell_date']  ?>"  id = "sdate"/>
...
<td colspan = 2 align = "center"> 
	  Name : <input type = "text" name = "cst_name"  value = "<?php   echo  $row_pinv['cust_name']  ?>"  class='auto'  size = "30"  />
 <input   name="custid"   id="custid"  value = "<?php   echo  $row_pinv['cust_id']  ?>"  type="hidden"   size = "5">	  
	...
	<?php 
		 echo"<option value = {$row_pinv['salesman_id']}>{$row_pinv['salesman_name']}</option>"; 
...
<option value = "<?php echo $accc_id ?>"><?php echo $accc_naame ?></option>
...
<input  class = "noPrint" type = "text"  width="30"   value = "<?php echo $id  ?>" name = "bill_id" id = "bill_id"  /> 



<input   class = "noPrint"  type = "text"  size = "5"   value = "1" name = "billstatus" id = "billstatus"  /> 

<input   class = "noPrint"   type = "text"  size = "5"   value = "<?php echo $accc_id ?>" name = "old_acc_id" id = "old_acc_id"  /> 
<input   class = "noPrint"  type = "text"  size = "5"   value = "<?php   echo  $row_pinv['balance']  ?>"   name = "old_acc_bal" id = "old_acc_bal"  />
<input   class = "noPrint"  type = "text"  size = "5"   value = "<?php   echo  $row_pinv['paid']  ?>"   name = "old_acc_paid" id = "old_acc_paid"  /> 
<input   name="oldcustid"   id="oldcustid"  value = "<?php   echo  $row_pinv['cust_id']  ?>"  type="hidden"   size = "5">
...
addRow_u('dataTable',"<?php echo  $row15['itemid'] ?>","<?php echo  $row15['itemname'] ?>","<?php echo  $row15['weight'] ?>","<?php echo  $row15['mrp'] ?>","<?php echo  $row15['sprice'] ?>","<?php echo  $row15['qty'] ?>","<?php echo  $row15['grpid'] ?>","<?php echo  $row15['tax_type'] ?>");  
