...
$result1 = mysql_query("SELECT DATE_FORMAT(sell_date,'%d/%m/%Y') as sell_date, `total`, `netpay`, `dis_rs`, `cust_id`, `cust_name`, `salesman_id`, `salesman_name`, `paid`, `balance`, `dis_pr`, `accid`, `billno`
 FROM `sell_invoice_2` WHERE   `billno` = $id");

 
$row1 = mysql_fetch_array($result1);

$result13 = mysql_query("SELECT `billno`, `itemid`, `qty`, `total`, `mrp`, `sprice`, `itemname`, `grpid` FROM `sell_items_2` WHERE billno = $id");
...
<tr>
<th style="text-align:center" colspan="10"  >  <span  style="font-size:22px; font-weight:bold;" > GURUNANAK DRYFRUITS </span> </br>
<span  style="font-size:10px; font-weight:bold;" >PATEL NAGAR  CITYCENTER , BEHIND S.P OFFICE GWALIOR 
      CONTACT : 0751 4073696  </span>
</th>
</tr>



<tr> <td align = "left"> Cash : MEMO  </td>   <td align = "right"> PARTY:  <?php echo $row1['cust_name']?> </td>   </tr>
...
TIMING 11  AMTO 10 PM , TUESDAY CLOSED
 </br>
