...
if($billstatus == "2" )
	...
	{
		$sell_inv_2 = "sales_return_main";
		$sell_item_2 = "sales_item_return";
...
$result31_s = mysql_query("SELECT `AUTO_INCREMENT` FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '$sDbName' AND TABLE_NAME = '$sell_inv_2' ");
...
$retcomm = trim(strip_tags(addslashes(strtoupper($_POST['retcomm']))));
...
$sqlinv = "INSERT INTO $sell_inv_2(`sell_date`, `total`, `netpay`, `dis_rs`, `cust_id`, `cust_name`, `salesman_id`, `salesman_name`, `paid`, `balance`, `dis_pr`, `accid`,retcom) VALUES
(STR_TO_DATE('{$_POST['sdate']}','%d/%m/%Y'),{$_POST['stotal']},'{$_POST['netpay']}','{$_POST['rsdis']}',
'$custid','$custname','{$_POST['sales_id']}','{$_POST['sales_name']}','{$_POST['paid']}','{$_POST['balance']}','{$_POST['sdis']}',$cashcard,'$retcomm')";
...
$sqlins1="INSERT INTO $sell_item_2(`billno`, `itemid`, `qty`, `total`, `mrp`, `sprice`, `itemname`, `grpid`,weight) VALUES
 ($billno,{$_POST['mastitem'.$i]},{$_POST['qty'.$i]},{$_POST['ttotal'.$i]},{$_POST['mrp'.$i]},{$_POST['sprice'.$i]},'{$_POST['mastdesc'.$i]}','{$_POST['grpid'.$i]}','{$_POST['weight1'.$i]}')"; 
...
while($row13 = mysql_fetch_array($result13))
  {
		...
		$sqlins2="INSERT INTO `store_usage_ret`(`billno`, `s_date`, `c_id`, `c_name`, `unit`, `qty`) VALUES 
...
header('Location: show_rec_bill_r.php?id='.$billno);
